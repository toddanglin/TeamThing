var TeamThing = function(kendoApp){
	...
	return {
		...
		loadThingsView: function(){
			...
			var lv = _eleThingList.kendoMobileListView({
		...
		updateThingStatus: function(tid, newStatus){
			var uid = this.getCurrentUser().id;
			
			return $.when(_data.updateThingStatus(tid, uid, newStatus)).then(function(result){
				if(result){
					return true;
				}else{
					return false;
